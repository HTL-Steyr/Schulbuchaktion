...
BEGIN
    ...
    IF (SELECT ebook_plus FROM book_order)
    THEN
    SELECT price_ebook_plus + (price_inclusive_ebook - book_price.price_ebook)
    FROM book_price
    WHERE book_price.book_id_id = (SELECT book_order.book_id_id FROM book_order WHERE id = last_insert_id())
        INTO pricePerBook;
    ELSE
        IF (SELECT ebook FROM book_order)
        THEN
...
END IF;
    ...
    SET totalPrice = pricePerBook * (SELECT count FROM book_order WHERE id = last_insert_id());

UPDATE book_order SET price=totalPrice WHERE id = last_insert_id();
